...
- **Fase atual:** 8 — Notificações Push (server-side completo, cliente aguarda google-services.json/APNs)
...
- [~] **7.5.** Streak quebrado: infra pronta via Edge Function `notify-trigger` aceitando `event=streak_broken`. Falta wiring: evaluator (`evaluate_environment_streaks`) chama o trigger quando `last_failed_at` muda. Sessão futura quando push estiver validado em device real.
...
- [~] **8.1.** `firebase_core` + `firebase_messaging` adicionados ao `pubspec.yaml`. `PushNotificationsService` (lib/data/services) faz init idempotente — engole erro se `google-services.json`/`GoogleService-Info.plist` ausentes (app não quebra). Aguardando config do Firebase para validar em device real.
