...
// Ninho — Fase 4.3: aceitar convite.
// IDEA.md §5.3 + §7.3.
//
// Fluxo:
//  1. Convidado autenticado envia o token claro.
//  2. Token foi gerado em `create-invite` como base64url de 32 bytes — aqui
//     desfazemos a codificação e calculamos o sha-256 dos mesmos bytes
//     originais (garante que o hash bata com o gravado em invites.token_hash).
//  3. RPC `accept_invite(token_hash)` faz checagem + insert atômico.
//
// Não logamos o token nem o hash. Erros do RPC viram códigos HTTP genéricos
// para não revelar existência do convite.
...
Deno.serve(async (req) => {
  ...
  if (error) {
    console.error("accept_invite failed", { code: error.code });
