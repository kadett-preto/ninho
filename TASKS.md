...
- **Fase atual:** 10 — Feed (timeline, detalhe, realtime, moderação e resumo semanal por IA implementados; eval suite pendente)
- **Última atualização:** 2026-05-22
- **Pendência aberta:** Fase 10 ainda precisa da eval suite real do resumo semanal (depende de `ANTHROPIC_API_KEY` + custo, fica para sessão dedicada — equivalente a 5.8).
...
- [~] **10.6.** Eval suite resumo semanal (§8.4) — snapshot test do SYSTEM_PROMPT (`test/weekly_summary_prompt_snapshot_test.dart`) + invariantes de §7.6/§7.8 já cobrem regressão silenciosa do prompt. Eval comportamental real (chamadas Claude com fixtures de ninhos diferentes) fica como pendência junto com 5.8 — depende de `ANTHROPIC_API_KEY` + custo.
